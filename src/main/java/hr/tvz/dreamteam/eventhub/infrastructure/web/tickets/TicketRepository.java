...
import hr.tvz.dreamteam.eventhub.infrastructure.web.statictics.model.RevenueStats;
import hr.tvz.dreamteam.eventhub.infrastructure.web.statictics.model.TicketStats;
...
public interface TicketRepository extends JpaRepository<Ticket, UUID> {
    ...
    @Query("SELECT new hr.tvz.dreamteam.eventhub.infrastructure.web.transactions.model.TicketDTO(t.event.id, t.category, t.price, COUNT(t)) FROM Ticket t WHERE t.event.id = :event_id AND t.status = 'FREE' GROUP BY t.event.id, t.category, t.price")
    List<TicketDTO> getAllGroupedByForEventId(UUID event_id);

    @Query("SELECT new hr.tvz.dreamteam.eventhub.infrastructure.web.transactions.model.TicketDTO(t.event.id, t.category, t.price, COUNT(t)) FROM Ticket t WHERE t.event.id = :event_id GROUP BY t.event.id, t.category, t.price")
    ...
    @Query("SELECT new hr.tvz.dreamteam.eventhub.infrastructure.web.tickets.model.EventTicketsDTO(t.id, t.category, t.price, t.status) " +
            "FROM Event e " +
